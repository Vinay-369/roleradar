...
async def main():
    ...
    print(f"Total Unique Live Active Indian Opportunities: {len(unique_india_active)}")
    
    role_counts = Counter()
    sr_count = 0
    
    for j in unique_india_active:
    ...
    print(f"\nSpecialized Requisition Count: {sr_count} (was 383)")
    print(f"Net remapped with evidence: {383 - sr_count}")
    print(f"Total unique roles covered: {len(role_counts)}")
    print("\nTop 20 roles by count:")
    for r, c in role_counts.most_common(20):
        print(f"  {r}: {c}")
